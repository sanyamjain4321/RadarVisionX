...
const Scene=(function(){
  ...
  function gen(){
    ...
    G('ov-d').textContent='0';G('ov-ai').textContent='—';G('ov-st').textContent='ready';
    _lastCallArgs = null;
  }
  ...
  function renderDets(cfarDetsOrig, aiFilteredOrig, extras={}){
    ...
    G('ov-d').textContent=cfarDetsOrig.length+(aiFilteredOrig?aiFilteredOrig.length:0);

    try {
      if (AIModel.isEnabled() && pipeline !== 'cfar') {
         G('ov-ai').textContent = cfarDetsOrig.length > 0 ? aiFilteredOrig?.length || 0 : 0;
      } else {
         G('ov-ai').textContent=cfarDetsOrig.length;
      }
    } catch (e) {
      console.error("AIModel error:", e);
      G('ov-ai').textContent=cfarDetsOrig.length;
