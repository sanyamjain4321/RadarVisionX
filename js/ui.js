...
async function runDetection(){
  ...
  try {
    ...
    await new Promise(r=>setTimeout(r,10));
    if(G('ov-cfar-ms')) G('ov-cfar-ms').textContent = tCFAR+'ms';
    ...
    if(pipeline === 'hybrid' || pipeline === 'yolo_backend'){
      ...
      if(!YOLOInference.isReady()){
        ...
        try {
          ...
          await new Promise(r=>setTimeout(r,10));
          if(G('ov-yolo'))    G('ov-yolo').textContent    = yoloDets.length;
          if(G('yolo-count')) G('yolo-count').textContent  = `${yoloDets.length}/${cfarDets.length}`;
          if(G('ov-yolo-ms')) G('ov-yolo-ms').textContent  = tYOLO+'ms';
      ...
      if(G('ov-yolo'))    G('ov-yolo').textContent    = '—';
      if(G('yolo-count')) G('yolo-count').textContent  = '—';
      if(G('ov-yolo-ms')) G('ov-yolo-ms').textContent  = '—';
    }
    ...
    await new Promise(r=>setTimeout(r,10));
    if(G('ov-ai-ms')) G('ov-ai-ms').textContent = tAI+'ms';
    ...
    G('ov-d').textContent  = cfarDets.length;
    G('ov-ai').textContent = kept.length;
...
function clearStats(){
  ...
  G('ov-d').textContent='0'; G('ov-ai').textContent='—'; G('ov-ms').textContent='—';
  G('ov-st').textContent='ready'; G('ov-yolo').textContent='—';
  ['ov-cfar-ms','ov-yolo-ms','ov-ai-ms'].forEach(id=>{ if(G(id)) G(id).textContent='—'; });
}
...
const RealSAR = (function(){
  ...
  function updateTable(cfar,ai){ const tbl=G('rs-table');if(!cfar.length){tbl.innerHTML='<span style="color:var(--dim)">No detections.</span>';return;}const aiIds=new Set();for(const ak of ai){for(let i=0;i<cfar.length;i++){if(cfar[i].x1===ak.x1&&cfar[i].y1===ak.y1){aiIds.add(i);break;}}}tbl.innerHTML=cfar.map((d,i)=>{const k=aiIds.has(i),c=k?'var(--green)':'var(--dim)';return`<div style="color:${c};border-bottom:1px solid var(--border);padding:2px 0">${k?'\u2713':'\u2717'} #${i+1} [${d.x1},${d.y1}\u2192${d.x2},${d.y2}] conf=${(d.conf*100).toFixed(0)}%</div>`;}).join(''); }
  ...
  function loadFile(file){
    ...
    if(isJSON){
      ...
      r.onload=e=>{
        try{
          ...
          rawDets=dets;G('rs-raw').textContent=dets.length;G('rs-cfar').textContent=dets.length;
          ...
          G('rs-ai').textContent=kept.length;drawDets(dets,kept);updateTable(dets,kept);
    ...
    reader.onload=e=>{
      ...
      img.onload=()=>{
        ...
        // ── AUTO PARAMETER COMPUTATION on upload ──────────────────
      ...
      img.onerror=()=>setStatus('\u274c Failed to load','var(--red)');
  ...
  async function runDetection(){
    ...
    // Use auto-computed winR/alpha if available, otherwise fall back to mode defaults
    ...
    rawDets=dets;G('rs-raw').textContent=dets.length;G('rs-cfar').textContent=dets.length;
    let kept=[],filtered=[];
    try{const r=AIModel.filter(dets,aiTh,W,H);kept=r.kept||dets;filtered=r.filtered||[];}
    catch(e){console.error('AI error',e);kept=dets;}
    G('rs-ai').textContent=kept.length;drawDets(dets,kept);updateTable(dets,kept);
    setStatus(`\u2705 ${kept.length} ships \u00b7 ${dets.length} CFAR \u00b7 ${ms}ms`,'var(--green)');
  }

  function loadDemo(){ const cv=G('rs-canvas'),W=860,H=500;cv.width=W;cv.height=H;const ctx=cv.getContext('2d');_colorMode=true;const imgData=ctx.createImageData(W,H),d=imgData.data;for(let i=0;i<W*H;i++){const u1=Math.random()||1e-9,u2=Math.random();let spk=Math.sqrt(-2*Math.log(u1))*Math.cos(2*Math.PI*u2)*6+12;spk=Math.max(0,spk);let r=Math.round(spk*0.6+5),g=Math.round(spk*0.9+18+Math.random()*4),b=Math.round(spk*2.2+55+Math.random()*8);const x=i%W,coastX=x<230,coastLine=x+(Math.floor(i/W))<360;if(coastX&&coastLine){const in2=Math.random()*60+30;r=Math.round(r+in2*0.1);g=Math.round(g+in2*0.7);b=Math.round(b+in2*0.6);}d[i*4]=Math.min(255,r);d[i*4+1]=Math.min(255,g);d[i*4+2]=Math.min(255,b);d[i*4+3]=255;}ctx.putImageData(imgData,0,0);const ships=[{x:420,y:180,rw:9,rh:5,a:0.2},{x:455,y:172,rw:8,rh:4,a:-0.1},{x:490,y:190,rw:10,rh:5,a:0.35},{x:440,y:210,rw:7,rh:4,a:0.05},{x:475,y:205,rw:9,rh:5,a:-0.25},{x:510,y:175,rw:8,rh:4,a:0.15},{x:530,y:200,rw:11,rh:5,a:0.45},{x:460,y:235,rw:7,rh:3,a:-0.3},{x:500,y:225,rw:9,rh:4,a:0.1},{x:545,y:220,rw:8,rh:4,a:0.25}];ships.forEach(s=>{ctx.save();ctx.translate(s.x,s.y);ctx.rotate(s.a);const gr=ctx.createRadialGradient(0,0,0,0,0,Math.max(s.rw,s.rh));gr.addColorStop(0,'rgba(255,255,200,1.0)');gr.addColorStop(0.35,'rgba(255,200,60,0.95)');gr.addColorStop(0.70,'rgba(240,140,20,0.75)');gr.addColorStop(1.0,'rgba(200,80,0,0.05)');ctx.fillStyle=gr;ctx.beginPath();ctx.ellipse(0,0,s.rw,s.rh,0,0,Math.PI*2);ctx.fill();ctx.restore();});G('rs-overlay').style.display='none';loadedImage={_demo:true,_colorMode:true};rawDets=ships.map(s=>({x1:Math.max(0,Math.round(s.x-s.rw*Math.abs(Math.cos(s.a))-s.rh*Math.abs(Math.sin(s.a))-2)),y1:Math.max(0,Math.round(s.y-s.rh*Math.abs(Math.cos(s.a))-s.rw*Math.abs(Math.sin(s.a))-2)),x2:Math.min(W,Math.round(s.x+s.rw*Math.abs(Math.cos(s.a))+s.rh*Math.abs(Math.sin(s.a))+2)),y2:Math.min(H,Math.round(s.y+s.rh*Math.abs(Math.cos(s.a))+s.rw*Math.abs(Math.sin(s.a))+2)),conf:+(0.62+Math.random()*0.34).toFixed(2),brightness:Math.round(80+Math.random()*60)}));G('rs-raw').textContent=rawDets.length;G('rs-cfar').textContent=rawDets.length;const aiTh=+G('rs-aith').value;let kept=[];try{kept=AIModel.filter(rawDets,aiTh,W,H).kept||rawDets;}catch(err){console.error(err);kept=rawDets;}G('rs-ai').textContent=kept.length;drawDets(rawDets,kept);updateTable(rawDets,kept);G('rs-run').disabled=true;setStatus(`\u2705 Demo SAR \u2014 ${rawDets.length} ships`,'var(--green)'); }
  return { loadFile, runDetection, loadDemo };
})();
